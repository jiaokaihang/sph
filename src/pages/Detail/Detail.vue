...
export default {
  ...
  mounted() {
    this.$store.dispatch("detailAbount/goodInfo", this.$route.params.id);
  },
  computed: {
    ...mapGetters("detailAbount", ["categoryView", "skuInfo"]),
    //给子组件的数据 如果skuInfo数据没有回来，那么这个对象数据应该是个空对象
    skuImageList() {
  ...
  methods: {
    ...
    //加入购物车的回调函数
    async addShopCart() {
      /*当前这里是派发一个action，也向服务器发起请求，判断加入购物车成功了还是失败了，进行相应的操作
      用try catch进行监控，
